        ...
        <table class="min-w-[600px] table-auto border-collapse border border-gray-200">
            ...
            <tbody id="tableBody">
                @foreach ($customers as $index => $customer)
                    <tr class="odd:bg-gray-50 even:bg-white">
                        ...
                        <td class="px-4 py-2 border border-gray-300 customer">
                            <span class="bg-info py-1 px-2 rounded font-bold text-xl text-white">{{ $customer->status }}</span>
                        </td>
                        ...
                        @if ($customer->status === 'meeting')
                            <form action="{{ route('customerStatus', $customer->id) }}" method="POST">
                                ...
                                <td class="flex gap-1 mt-4">
                                    ...
                                    <button class="btn btn-success" id="statusBtn">meeting done</button>
                                </td>
                            </form>
                ...
                @if ($customers->isEmpty())
                    <tr>
                        <td colspan="11" class="text-center">No Meeting Record Found</td>
                    </tr>
                @endif
            </tbody>
        </table>
